...
**Stage 1 (real content + Constitution reader) is in progress.** A Constitution reader is bundled and working: Chapters I, II, III, IV and VIII (ss. 1–32, 33–46, and 297–320 — General Provisions, Fundamental Objectives, Citizenship, Fundamental Rights, and FCT/General Supplementary Provisions) are real, verbatim Official Gazette text — cross-checked section-by-section against an independent, government-cited transcription (details and sourcing in `src/db/seed-data.ts`) — browsable by chapter/section and searchable fully offline (FTS5 on native, with an automatic plain-SQL fallback on platforms like web where the fts5 module isn't available). Chapters V, VI and VII (The Legislature, The Executive, The Judicature — around 250 sections combined) aren't bundled yet, and the app says so rather than implying completeness. Emergency contact numbers were updated from explicit fake placeholders to each agency's real, currently-published number (sources cited in `src/db/seed-data.ts`) — they're still marked `is_verified: 0` pending the ops/legal sign-off step (confirming with each agency this is still their live line) that safety-critical content needs before shipping.
...
| **1 — Real content + Constitution reader** | Legally reviewed statute content & verified emergency contacts · state-specific emergency numbers (mental health helplines, per-state services — see [Nigerian Mental Health Helplines](https://www.nigerianmentalhealth.org/helplines), [LagosMind Emergency Services](https://lagosmind.org/emergency-services)) · full Constitution of Nigeria bundled into SQLite, readable & searchable offline (FTS5) · complete EN/Pidgin content | 🚧 In progress — Constitution reader shipped (Chapters I, II, III, IV & VIII) + real national-level emergency contact numbers; state-specific numbers, Chapters V/VI/VII (Legislature, Executive, Judicature), and Q&A content review still pending |
